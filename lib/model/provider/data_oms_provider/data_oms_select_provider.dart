...
class DataOmsSelectProvider with ChangeNotifier{
  ...
  //---------------- Data info vehicule
      ...
      VehicleTypeResp? getVehicleTypeRespById(int? code){
        ...
        return vehicleTypeRespListSelect?.firstWhere((element) => element?.id == code, orElse: () => null);
      }
      ...
      VehicleModelResp? getVehicleModelRespById(int? code){
        ...
        return vehicleModelRespListSelect?.firstWhere((element) => element?.id == code, orElse: () => null);
      }
  ...
  UpdateDataOmsSelectProvider({BuildContext? context}) async{
    ...
    await SelectDataOmsRequest(context: context).respSelectDataOmsRequest().then((jsonData) async {
      ...
      //-------------------------------  Take Data Oms Person
        ...
        dataSelect= allDataSelectOms?["occupantRestraintSystemResp"].cast<Map<String, dynamic>>();
        occupantRestraintSystemRespListSelect = dataSelect.map((oneMap) => OccupantRestraintSystemResp.fromMap(oneMap)).toList();

        dataSelect= allDataSelectOms?["wearingHelmetResp"].cast<Map<String, dynamic>>();
        wearingHelmetRespListSelect = dataSelect.map((oneMap) => WearingHelmetResp.fromMap(oneMap)).toList();
